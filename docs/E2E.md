...
| Embedding vector | **Encrypted at rest** (server key); plain in RAM during search | **Yes at search time** | Semantic search, duplicates, resurfacing — see "Embedding privacy v2" below for the planned fix |
...
3. Seed → deterministic k×d random matrix `R` (k=256 bits, d=384 for BGE-small), generated by seeding a PRNG from step 2. **Never stored or transmitted** — regenerated on the fly every time it's needed, same as the vault key itself.
...
**Search**: server does Hamming distance (XOR + popcount) between the query's fingerprint and all of a user's stored fingerprints, returns top-K candidates. Client decrypts those K, re-embeds the real text locally, and re-ranks by true cosine similarity for the final precise order (replacing today's server-side exact cosine rank with an approximate-then-exact two-step, same shape as the existing full-text false-positive pass).

**Implementation checklist (not started):**
- [ ] Client: HKDF-expand vault key → matrix seed; seeded PRNG → matrix generation; project/binarize/pack helpers.
- [ ] API: capture create/update send the packed fingerprint instead of a raw embedding.
- [ ] Server: store fingerprint bytes as-is (new field, replaces `EncryptedEmbedding`); Hamming-distance ranking in Go (XOR + popcount) replaces `rankCapturesBySimilarity`'s cosine-on-decrypted-vector step.
- [ ] Server: client re-fetches top-K ciphertexts, re-embeds locally, re-ranks exactly (extends the existing hybrid-search merge pattern in `notesApi.js`).
- [ ] Remove `web/embedding_crypto.go`, `NOTEIKA_EMBEDDING_KEY`, `db.Capture.EncryptedEmbedding`/`LegacyEmbedding` entirely (no migration needed — no real users yet).
...
- [ ] Embeddings: binarized fingerprints, remove server embedding key — see "Embedding privacy v2" above.
